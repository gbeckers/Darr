...
v = np.memmap('values/arrayvalues.bin', dtype='<f2', shape=(26, 2), order='C')
def get_subarray(k):
    starti, endi = i[k]
...
v = readBin(con=fileid, what=numeric(), n=52, size=2, signed=TRUE, endian="little")
...
close(fileid);
# read float16 values array:
fileid = open("values/arrayvalues.bin","r");
v = map(ltoh, read!(fileid, Array{Float16}(undef, 2, 26)));
close(fileid);
...
(* read float16 values array: *)
None(* create a function that returns the k-th subarray
   from the values array *):
