...
---------------------------
import numpy as np
a = np.memmap('arrayvalues.bin', dtype='<u4', shape=(26, 2), order='C')

R:
--
fileid = file("arrayvalues.bin", "rb")
a = readBin(con=fileid, what=integer(), n=52, size=4, signed=FALSE, endian="little")
...
a = fread(fileid, [2, 26], '*uint32', 'ieee-le');
fclose(fileid);

Julia (version < 1.0):
----------------------
fileid = open("arrayvalues.bin","r");
a = map(ltoh, read(fileid, UInt32, (2, 26)));
close(fileid);

Julia (version >= 1.0):
-----------------------
fileid = open("arrayvalues.bin","r");
a = map(ltoh, read!(fileid, Array{UInt32}(undef, 2, 26)));
close(fileid);

IDL/GDL:
--------
a = read_binary("arrayvalues.bin", data_type=13, data_dims=[2, 26], endian="little")
