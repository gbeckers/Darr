...
---------------------------
import numpy as np
a = np.memmap('arrayvalues.bin', dtype='<u2', shape=(7,), order='C')

R:
--
fileid = file("arrayvalues.bin", "rb")
a = readBin(con=fileid, what=integer(), n=7, size=2, signed=FALSE, endian="little")
close(fileid)
...
a = map(ltoh, read(fileid, UInt16, (7,)));
close(fileid);

Julia (version >= 1.0):
-----------------------
fileid = open("arrayvalues.bin","r");
a = map(ltoh, read!(fileid, Array{UInt16}(undef, 7)));
close(fileid);

IDL/GDL:
--------
a = read_binary("arrayvalues.bin", data_type=12, data_dims=[7], endian="little")
