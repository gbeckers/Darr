...
v = np.memmap('values/arrayvalues.bin', dtype='<u2', shape=(26, 2), order='C')
def getsubarray(k):
    starti, endi = i[k]
...
v <- readBin(con=fileid, what=integer(), n=52, size=2, signed=FALSE, endian="little")
...
v = read_binary("values/arrayvalues.bin", data_type=12, data_dims=[2, 26], endian="little")
...
v = map(ltoh, read!(fileid, Array{UInt16}(undef, 2, 26)));
close(fileid);
...
v = fread(fileid, [2, 26], '*uint16', 'ieee-le');
fclose(fileid);
...
(* read uint16 values array: *)
v = BinaryReadList["values/arrayvalues.bin", "UnsignedInteger16", ByteOrdering -> -1];
