...
and can vary in the length of its first dimension. The array consists of uint8
numbers. 
...
v = np.memmap('values/arrayvalues.bin', dtype='<u1', shape=(26, 2), order='C')
def getsubarray(k):
    starti, endi = i[k]
...
v <- readBin(con=fileid, what=integer(), n=52, size=1, signed=FALSE, endian="little")
...
; read uint8 values array:
v = read_binary("values/arrayvalues.bin", data_type=1, data_dims=[2, 26], endian="little")
...
v = map(ltoh, read!(fileid, Array{UInt8}(undef, 2, 26)));
close(fileid);
...
v = fread(fileid, [2, 26], '*uint8', 'ieee-le');
fclose(fileid);
...
(* read uint8 values array: *)
v = BinaryReadList["values/arrayvalues.bin", "UnsignedInteger8", ByteOrdering -> -1];
