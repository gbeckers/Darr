...
v = np.memmap('values/arrayvalues.bin', dtype='<c8', shape=(26, 2), order='C')
def getsubarray(k):
    starti, endi = i[k]
...
; read complex64 values array:
v = read_binary("values/arrayvalues.bin", data_type=6, data_dims=[2, 26], endian="little")
...
v = map(ltoh, read!(fileid, Array{Complex{Float32}}(undef, 2, 26)));
close(fileid);
...
(* read complex64 values array: *)
v = BinaryReadList["values/arrayvalues.bin", "Complex64", ByteOrdering -> -1];
