...
will lead to an array that has its dimensions reversed (2, 9) with respect to
...
a = a.reshape((9, 2), order='C')

Python with Numpy (memmap):
---------------------------
import numpy as np
a = np.memmap('arrayvalues.bin', dtype='<i8', shape=(9, 2), order='C')

Scilab:
-------
fileid = mopen("arrayvalues.bin", "rb");
a = mgeti(18, "ll", fileid);
a = matrix(a, [2, 9]);
mclose(fileid);
...
a = fread(fileid, [2, 9], '*int64', 'ieee-le');
fclose(fileid);

Julia (version < 1.0):
----------------------
fileid = open("arrayvalues.bin","r");
a = map(ltoh, read(fileid, Int64, (2, 9)));
close(fileid);

Julia (version >= 1.0):
-----------------------
fileid = open("arrayvalues.bin","r");
a = map(ltoh, read!(fileid, Array{Int64}(undef, 2, 9)));
close(fileid);

IDL/GDL:
--------
a = read_binary("arrayvalues.bin", data_type=14, data_dims=[2, 9], endian="little")

Mathematica:
------------
a = BinaryReadList["arrayvalues.bin", "Integer64", ByteOrdering -> -1];
a = ArrayReshape[a, {9, 2}];

Maple:
------
a := FileTools[Binary][Read]("arrayvalues.bin", integer[8], byteorder=little, output=Array);
FileTools[Binary][Close]("arrayvalues.bin");
a := ArrayTools[Reshape](a, [2, 9]);
