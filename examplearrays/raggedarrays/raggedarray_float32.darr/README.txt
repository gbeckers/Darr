...
v = np.memmap('values/arrayvalues.bin', dtype='<f4', shape=(26, 2), order='C')
def getsubarray(k):
    starti, endi = i[k]
...
v <- readBin(con=fileid, what=numeric(), n=52, size=4, signed=TRUE, endian="little")
...
; read float32 values array:
v = read_binary("values/arrayvalues.bin", data_type=4, data_dims=[2, 26], endian="little")
...
v = map(ltoh, read!(fileid, Array{Float32}(undef, 2, 26)));
close(fileid);
...
# read float32 values array:
v := FileTools[Binary][Read]("values/arrayvalues.bin", float[4], byteorder=little, output=Array);
...
v = fread(fileid, [2, 26], '*float32', 'ieee-le');
fclose(fileid);
...
(* read float32 values array: *)
v = BinaryReadList["values/arrayvalues.bin", "Real32", ByteOrdering -> -1];
