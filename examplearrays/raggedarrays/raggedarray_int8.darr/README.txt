...
and can vary in the length of its first dimension. The array consists of int8
numbers. 
...
v <- readBin(con=fileid, what=integer(), n=52, size=1, signed=TRUE, endian="little")
...
v = map(ltoh, read!(fileid, Array{Int8}(undef, 2, 26)));
close(fileid);
...
# read int8 values array:
v := FileTools[Binary][Read]("values/arrayvalues.bin", integer[1], byteorder=little, output=Array);
...
v = fread(fileid, [2, 26], '*int8', 'ieee-le');
fclose(fileid);
