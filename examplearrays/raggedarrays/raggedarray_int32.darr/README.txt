...
and can vary in the length of its first dimension. The array consists of int32
numbers. 
...
v = np.memmap('values/arrayvalues.bin', dtype='<i4', shape=(26, 2), order='C')
def getsubarray(k):
    starti, endi = i[k]
...
i <- readBin(con=fileid, what=integer(), n=18, size=8, signed=TRUE, endian="little")
i <- array(data=i, dim=c(2, 9), dimnames=NULL)
close(fileid)
# read array of values:
fileid <- file("values/arrayvalues.bin", "rb")
v <- readBin(con=fileid, what=integer(), n=52, size=4, signed=TRUE, endian="little")
...
getsubarray <- function(k){
    ...
    if (starti > endi) {
        return (array(numeric(),c(2,0))) # empty array
    } else {
        return (v[,starti:endi])
    }
}
...
; read int32 values array:
v = read_binary("values/arrayvalues.bin", data_type=3, data_dims=[2, 26], endian="little")
...
v = map(ltoh, read!(fileid, Array{Int32}(undef, 2, 26)));
close(fileid);
...
------
# read indices array, to be used on values array later:
i := FileTools[Binary][Read]("indices/arrayvalues.bin", integer[8], byteorder=little, output=Array);
FileTools[Binary][Close]("indices/arrayvalues.bin");
i := ArrayTools[Reshape](i, [2, 9]);
# read int32 values array:
v := FileTools[Binary][Read]("values/arrayvalues.bin", integer[4], byteorder=little, output=Array);
FileTools[Binary][Close]("values/arrayvalues.bin");
v := ArrayTools[Reshape](v, [2, 26]);
# create a function that returns the k-th subarray
# from the values array:
getsubarray := proc (k::integer);
    v(.., i(1,k) + 1 .. i(2,k));
end proc;
# example to read third (k=3) subarray:
sa = getsubarray(3);
...
v = fread(fileid, [2, 26], '*int32', 'ieee-le');
fclose(fileid);
...
(* read int32 values array: *)
v = BinaryReadList["values/arrayvalues.bin", "Integer32", ByteOrdering -> -1];
