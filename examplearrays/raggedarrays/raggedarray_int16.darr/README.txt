...
and can vary in the length of its first dimension. The array consists of int16
numbers. 
...
v = np.memmap('values/arrayvalues.bin', dtype='<i2', shape=(26, 2), order='C')
def getsubarray(k):
    starti, endi = i[k]
...
v <- readBin(con=fileid, what=integer(), n=52, size=2, signed=TRUE, endian="little")
...
; read int16 values array:
v = read_binary("values/arrayvalues.bin", data_type=2, data_dims=[2, 26], endian="little")
...
v = map(ltoh, read!(fileid, Array{Int16}(undef, 2, 26)));
close(fileid);
...
# read int16 values array:
v := FileTools[Binary][Read]("values/arrayvalues.bin", integer[2], byteorder=little, output=Array);
...
v = fread(fileid, [2, 26], '*int16', 'ieee-le');
fclose(fileid);
...
(* read int16 values array: *)
v = BinaryReadList["values/arrayvalues.bin", "Integer16", ByteOrdering -> -1];
