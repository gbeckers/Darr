...
and can vary in the length of its first dimension. The array consists of int64
numbers. 
...
v = np.memmap('values/arrayvalues.bin', dtype='<i8', shape=(26, 2), order='C')
def getsubarray(k):
    starti, endi = i[k]
...
; read int64 values array:
v = read_binary("values/arrayvalues.bin", data_type=14, data_dims=[2, 26], endian="little")
...
v = map(ltoh, read!(fileid, Array{Int64}(undef, 2, 26)));
close(fileid);
...
# read int64 values array:
v := FileTools[Binary][Read]("values/arrayvalues.bin", integer[8], byteorder=little, output=Array);
...
v = fread(fileid, [2, 26], '*int64', 'ieee-le');
fclose(fileid);
...
(* read int64 values array: *)
v = BinaryReadList["values/arrayvalues.bin", "Integer64", ByteOrdering -> -1];
