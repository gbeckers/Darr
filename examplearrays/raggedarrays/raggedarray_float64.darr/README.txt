...
v = np.memmap('values/arrayvalues.bin', dtype='<f8', shape=(26, 2), order='C')
def getsubarray(k):
    starti, endi = i[k]
...
v <- readBin(con=fileid, what=numeric(), n=52, size=8, signed=TRUE, endian="little")
...
; read float64 values array:
v = read_binary("values/arrayvalues.bin", data_type=5, data_dims=[2, 26], endian="little")
...
v = map(ltoh, read!(fileid, Array{Float64}(undef, 2, 26)));
close(fileid);
...
# read float64 values array:
v := FileTools[Binary][Read]("values/arrayvalues.bin", float[8], byteorder=little, output=Array);
...
v = fread(fileid, [2, 26], '*float64', 'ieee-le');
fclose(fileid);
...
(* read float64 values array: *)
v = BinaryReadList["values/arrayvalues.bin", "Real64", ByteOrdering -> -1];
