...
v = np.memmap('values/arrayvalues.bin', dtype='<u4', shape=(26, 2), order='C')
def getsubarray(k):
    starti, endi = i[k]
...
; read uint32 values array:
v = read_binary("values/arrayvalues.bin", data_type=13, data_dims=[2, 26], endian="little")
...
v = map(ltoh, read!(fileid, Array{UInt32}(undef, 2, 26)));
close(fileid);
...
v = fread(fileid, [2, 26], '*uint32', 'ieee-le');
fclose(fileid);
...
(* read uint32 values array: *)
v = BinaryReadList["values/arrayvalues.bin", "UnsignedInteger32", ByteOrdering -> -1];
