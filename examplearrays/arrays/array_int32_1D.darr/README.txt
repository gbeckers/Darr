...
---------------------------
import numpy as np
a = np.memmap('arrayvalues.bin', dtype='<i4', shape=(9,), order='C')

R:
--
fileid <- file("arrayvalues.bin", "rb")
a <- readBin(con=fileid, what=integer(), n=9, size=4, signed=TRUE, endian="little")
close(fileid)
...
a = fread(fileid, 9, '*int32', 'ieee-le');
fclose(fileid);

Julia (version < 1.0):
----------------------
fileid = open("arrayvalues.bin","r");
a = map(ltoh, read(fileid, Int32, (9,)));
close(fileid);

Julia (version >= 1.0):
-----------------------
fileid = open("arrayvalues.bin","r");
a = map(ltoh, read!(fileid, Array{Int32}(undef, 9)));
close(fileid);

IDL/GDL:
--------
a = read_binary("arrayvalues.bin", data_type=3, data_dims=[9], endian="little")
